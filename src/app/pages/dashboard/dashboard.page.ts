...
export class DashboardPage extends BasePage {
	...
	constructor(public navController: NavController,
		...
		super(navController, null, communicator, menuController, platform, router, uxNotifierService, userTypesService, featureService, themeableBrowser);
	...
	private async refreshPropertiesAsync() {
		...
		for (let userType of this.User.Types) {
			await this.userDetailsService.getProperties(userType.Id)
				.then(
	...
	private async getInteriorAreas() {
		await this.propertyService.getAreaTypes('interior').then(
			(response: any) => {
	...
	private async getBedrooms() {
		this.propertyService.getAreaTypes('bedroom').then(
			(response: Array<any>) => {
	...
	private async getBathroomAreas() {
		this.propertyService.getAreaTypes('bathroom').then(
			(response: Array<any>) => {
	...
	private getSelectedProperty() {
		...
		this._selectedPropertySubscription = this.communicator.getSelectedProperty().subscribe(
			(property) => {
	...
	private async getAllUserProperties() {
		...
		try {
					...
					await this.userDetailsService.getProperties(userType.Id)
						.then(
							(properties: Array<INewPropertyDto>) => {
									...
									if (this.ActiveProperty !== null) {
										if (this.ActiveProperty.IsProxy) {
											...
											if (this.ActiveProperty.Profiles == undefined || this.ActiveProperty.Profiles == null || this.ActiveProperty.Profiles.length == 0) {
												this.userDetailsService.getProperty(this.ActiveProperty.Id).then((x: any) => {
													this.setupProperty(x);

													this.AppInsights.trackEvent({
														name: 'LoadingProperties', properties: [
															{
																"userID": this.User.Id
															},
															{
								...
								this.AppInsights.trackEvent({
									name: 'LoadingProperties-Done', properties: [
										{
											"userID": this.User.Id
										}
									]
							...
							(err) => {
								this.AppInsights.trackEvent({
									name: 'LoadingProperties-Error', properties: [
										{
											"userID": this.User.Id
										}
									]
			...
			this.AppInsights.trackEvent({
				name: 'end: getAllUserProperties()', properties: [
					{
						"userID": this.User.Id
					}
				]
	...
	public async getActivePropertyAreas(refreshProperty: boolean = false) {
		...
		if (this.ActiveProperty.Profiles === undefined ||
			this.ActiveProperty.Profiles === null ||
			this.ActiveProperty.Profiles.length === 0 || refreshProperty) {
			await this.userDetailsService.getProperty(this.ActiveProperty.Id).then((x: any) => {
	...
	private async displaySuite16Categories() {
		...
		this.data.Lists = [
			{
				Name: '',
				Items: this.Suite16Categories.map(
					(x: ISuite16CategoryDto) => {
						let icon: string = '';
	...
	public async getSuite16Categories() {
		if (this.Suite16Categories == undefined || this.Suite16Categories == null || this.Suite16Categories.length == 0) {
			await this.suite16CategoryService.getSuite16Categories()
				.then(
					...
					(err) => {
						...
						if (this.Suite16Categories === undefined
