...
export class PropertyProfileExteriorAreasPage extends BasePage {
	...
	async getExteriors() {
		...
		await this.prePreConstruction.getAreaTypes('exterior')
			.then(
				(response: any) => {
					if (response) {
	...
	async saveProperty() {
		...
		if (this.UserTypes.filter(x => x.Name != this.constants.UserTypes.Unassigned || x.Name != this.constants.UserTypes.Owner)
			.some(x => x.Id === this.NewSelectedUserTypeId)){
			// figure out which type to do custom stuff???
			this.companyInformationService.upsertCompanyInformationAsync(this.CompanyInformation)
				.then((x) => {
		...
		if (this.IsNewUserTypeSelected === true) {
			this.userTypesService.saveNewUserType(this.NewSelectedUserTypeId)
				.then(async (i) => {
	...
	private async save(loader: any, customProperty: IPropertyDto) {
		debugger;
		this.privatelabelService.saveCustomPropertyAsync(customProperty).then(async (savedProperty:any) => {
			loader.dismiss();
			this.closeAfterSave();
