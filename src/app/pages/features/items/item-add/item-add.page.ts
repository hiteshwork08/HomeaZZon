...
export class ItemAddPage extends BasePage {
	...
	constructor(public navCtrl: NavController, public platform: Platform,
		public propertyProfilesService: PropertyProfilesService, private loadingController: LoadingController,
		public searchService: SearchService, public uxNotifierService: UxNotifierService, public router: Router) {
		...
		if (this.bookmarkUrl != undefined
			&& this.bookmarkUrl != null && this.bookmarkUrl != '') {
	...
	private async getInteriorAreas() {
		await this.propertyProfilesService.getAreaTypes('interior').then(
			(response: any) => {
	...
	private async getBedrooms() {
		this.propertyProfilesService.getAreaTypes('bedroom').then(
			(response: Array<any>) => {
	...
	private async getBathroomAreas() {
		this.propertyProfilesService.getAreaTypes('bathroom').then(
			(response: Array<any>) => {
	...
	private async getProfileItemLineItems(selectedProfileItem: any, userType: string) {
		...
		await this.propertyProfilesService.getProfileItems(selectedProfileItem.Id, userType)
			.then(
				async (response: any) => {
					selectedProfileItem.Lineitems = [];

					response.Area.LineItems.map(
						(x) => {
