...
export class ItemsPage extends BasePage {
	...
	constructor(public navController: NavController,
		...
		super(navController, navParams, communicator, menuController, platform, router, uxNotifierService, userTypesService, null, themeableBrowser);
	...
	private async getDynamicViewResultsAsync(id: number = 0) {
		...
		// 2. 
		//this.LineItem.View.
		this.viewsProvider.getViewResultsAsync({
			ViewSegmentId: id,
		...
		if (this.LineItem.Id == undefined) {
			...
			if (this.LineItem.Name == '01.03 Architectural') {
				...
				if (this.LineItem.IsView) {
					...
					// 1. Google Shopping
					await this.searchService.getSavedGoogleProductData(this.ActiveProperty.Id, profileItemId, this.LineItem.Id, 0, this.ActiveProperty.IsProxy)
						.then(
							(x: Array<SavedProductSearchResultDto>) => {
								x.map(
									(result) => {
										...
										if (result.IsSuggest) {
											googleProductGridListsSuggest.Items.push({ Id: result.Id, Name: result.Name, IconPath: null, ImagePath: result.Image, IsUnopenedSuggestion: result.IsUnopenedSuggestion });
											suggestedCount += 1;
										}
									}
								);
					...
					// 2. Amazon
					await this.searchService.getSavedAmazonData(this.ActiveProperty.Id, profileItemId, this.LineItem.Id, 0, this.ActiveProperty.IsProxy)
						.then(
							(x: Array<SavedProductSearchResultDto>) => {
								x.map(
									(result) => {
										...
										if (result.IsSuggest) {
											amazonGridListsSuggest.Items.push({ Id: result.Id, Name: result.Name, IconPath: null, ImagePath: result.Image, IsUnopenedSuggestion: result.IsUnopenedSuggestion });
											suggestedCount += 1;
										}
									}
								);
					...
					// 3. Google Web
					await this.searchService.getSavedGoogleData(this.ActiveProperty.Id, profileItemId, this.LineItem.Id, 0, this.ActiveProperty.IsProxy)
						.then(
							(x: Array<SavedSearchEngineSearchResultDto>) => {
								x.map(
									(result: SavedSearchEngineSearchResultDto) => {
										if (result.IsMy) {
										...
										if (result.IsSuggest) {
											googleWebGridListsSuggest.Items.push({ Id: result.Id, Name: result.Title, IconPath: null, ImagePath: null, IsUnopenedSuggestion: result.IsUnopenedSuggestion });
											suggestedCount += 1;
										}
									}
								);
					...
					// 4. YouTube
					await this.searchService.getSavedYouTubeData(this.ActiveProperty.Id, profileItemId, this.LineItem.Id, 0, this.ActiveProperty.IsProxy)
						.then(
							(x: Array<SavedYouTubeSearchResultDto>) => {
								x.map(
									(result: SavedYouTubeSearchResultDto) => {
										...
										if (result.IsSuggest) {
											youTubeGridListsSuggest.Items.push({ Id: result.Id, Name: result.Title, IconPath: null, ImagePath: result.ThumbnailImg, IsUnopenedSuggestion: result.IsUnopenedSuggestion });
											suggestedCount += 1;
										}
									}
								);
					...
					// 5. DigiDoc
					await this.itemService.getDigiDocs(this.ActiveProperty.Id, profileItemId, this.LineItem.Id, 0, this.ActiveProperty.IsProxy)
						.then(
							(x: ITupleResult) => {
								...
								x.Item1.map(
									(result: IDigiDocDto) => {
										...
										if (result.ContentType === "application/pdf") {
											iconUrl = "https://firebasestorage.googleapis.com/v0/b/itt-content.appspot.com/o/Common%2Fassets%2Fsvgs%2Fregular%2Ffile-pdf.svg?alt=media&token=ec5d1f8a-8393-4b64-9a5b-5cc75ee01660";
										} else if (result.ContentType === "application/msword" || result.ContentType == "application/vnd.openxmlformats-officedocument.wordprocessingml.document") {
											iconUrl = "https://firebasestorage.googleapis.com/v0/b/itt-content.appspot.com/o/Common%2Fassets%2Fsvgs%2Fregular%2Ffile-word.svg?alt=media&token=f2f34448-fefe-4cbe-bcc7-0e2fe9ae0ef2";
										} else if (result.ContentType === "application/vnd.ms-excel" || result.ContentType == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
											iconUrl = "https://firebasestorage.googleapis.com/v0/b/itt-content.appspot.com/o/Common%2Fassets%2Fsvgs%2Fregular%2Ffile-excel.svg?alt=media&token=853b9d8f-3040-4540-8e7a-a33da225e793";
										} else if (result.ContentType === "application/vnd.ms-powerpoint" || result.ContentType == "application/vnd.openxmlformats-officedocument.presentationml.presentation") {
											iconUrl = "https://firebasestorage.googleapis.com/v0/b/itt-content.appspot.com/o/Common%2Fassets%2Fsvgs%2Fregular%2Ffile-powerpoint.svg?alt=media&token=a603f641-db7d-408c-8622-ed0c628cbe7e";
										}

										if (result.IsMy) {
											if (!this.IsInArray(result.Id, taggedArtifacts)) {
												digiDocGridListsMy.Items.push({ Id: result.Id, Name: result.AssetInfo.Title, IconPath: iconUrl, ImagePath: result.Url, ContentType: result.ContentType });
											}
											myCount += 1;
										}
										if (result.IsWishlist) {
											if (!this.IsInArray(result.Id, taggedArtifacts)) {
												digiDocGridListsWishlist.Items.push({ Id: result.Id, Name: result.AssetInfo.Title, IconPath: iconUrl, ImagePath: result.Url, ContentType: result.ContentType });
											}
											wishlistCount += 1;
										}
										if (result.IsSuggest) {
											if (!this.IsInArray(result.Id, taggedArtifacts)) {
												digiDocGridListsSuggest.Items.push({ Id: result.Id, Name: result.AssetInfo.Title, IconPath: iconUrl, ImagePath: result.Url, IsUnopenedSuggestion: result.IsUnopenedSuggestion, ContentType: result.ContentType });
											}
											suggestedCount += 1;
										}
									}
								...
								digiDocGridListsTags.Items.forEach((x) => {
									if (x.ArtifactIds !== undefined &&
										x.ArtifactIds !== null &&
										x.ArtifactIds.some(y => y)) {
					...
					// 6. Upc
					await this.itemService.getUpcProducts(this.ActiveProperty.Id, profileItemId, this.LineItem.Id, 0, this.ActiveProperty.IsProxy)
						.then(
							(x: Array<IProductDto>) => {
								x.map(
									(result: IProductDto) => {
										...
										if (result.IsSuggest) {
											upcGridListsSuggest.Items.push({ Id: result.Id, Name: result.AssetInfo.Title, IconPath: null, ImagePath: result.Image, IsUnopenedSuggestion: result.IsUnopenedSuggestion });
											suggestedCount += 1;
										}
									}
								);
					...
					// 7. QrCode
					await this.itemService.getQrCodes(this.ActiveProperty.Id, profileItemId, this.LineItem.Id, 0, this.ActiveProperty.IsProxy)
						.then(
							(x: Array<IQrCodeDto>) => {
								x.map(
									(result: IQrCodeDto) => {
										if (result.IsMy) {
											qrCodeGridListsMy.Items.push({ Id: result.Id, Name: result.Url, IconPath: null, ImagePath: 'https://firebasestorage.googleapis.com/v0/b/homeazzon.appspot.com/o/common%2Ffont-awesome%2Fregular%2Fqrcode.svg?alt=media&token=d2f01c14-b5b7-4248-b666-0fcd6584c0a6' });
											myCount += 1;
										}
										if (result.IsWishlist) {
											qrCodeGridListsWishlist.Items.push({ Id: result.Id, Name: result.Url, IconPath: null, ImagePath: 'https://firebasestorage.googleapis.com/v0/b/homeazzon.appspot.com/o/common%2Ffont-awesome%2Fregular%2Fqrcode.svg?alt=media&token=d2f01c14-b5b7-4248-b666-0fcd6584c0a6' });
											wishlistCount += 1;
										}
										if (result.IsSuggest) {
											qrCodeGridListsSuggest.Items.push({
												...
												Name: result.Url,
												IconPath: null,
												ImagePath: 'https://firebasestorage.googleapis.com/v0/b/homeazzon.appspot.com/o/common%2Ffont-awesome%2Fregular%2Fqrcode.svg?alt=media&token=d2f01c14-b5b7-4248-b666-0fcd6584c0a6',
	...
	async dynamicTabClickHandler(id: number) {
		...
		this.data = { Lists: [] };
		this._genericViewResultGrid = { Lists: [] };
		await this.getDynamicViewResultsAsync(id).then((x) => {
			this._loading.dismiss();
		}, (err) => { });;
