...
export class SearchPage extends BasePage {
	...
	constructor(public navController: NavController,
		...
		this.ActiveProperty.Profiles.map(
			(x) => {
				x.ProfileItems.map(
					(y) => {
	...
	public search() {
		...
		let navExtras: NavigationExtras = {
			queryParams: {
				searchSource: this.searchSource, keyword: keywords
			}
		};
	...
	public async getLineitems() {
		...
		if (this.selectedProfileItem) {
			...
			await this.propertyService.getProfileItems(this.selectedProfileItem.Id, this.User.Types[0].Name)
				.then(
					async (response: any) => {
