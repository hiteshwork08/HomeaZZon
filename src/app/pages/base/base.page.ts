...
export class BasePage implements OnInit {
	...
	constructor(public navController: NavController,
		...
		public themeableBrowser: ThemeableBrowser = null) {
	...
	public bookmark() {
		...
		try {
			...
			if (this.platform.is('mobileweb')) {
				...
				const browser: ThemeableBrowserObject = this.themeableBrowser.create('https://www.google.com', '_blank', options);

				browser.on('home').subscribe(() => {
					browser.executeScript({
						code: "window.location.href ='http://www.google.com';"
					});
				});

				browser.on('bookmark').subscribe((x: any) => {
					browser.close();
				...
				browser.on('browse').subscribe((x: any) => {
					browser.executeScript({
